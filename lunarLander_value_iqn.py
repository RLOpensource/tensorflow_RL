...
model_path = 'iqn/model'
sess = tf.Session()
state_size, output_size, num_support, batch_size = 8, 4, 8, 8
mainNet = MLPIQN('main', state_size, output_size, num_support, batch_size)
targetNet = MLPIQN('target', state_size, output_size, num_support, batch_size)
agent = IQN(sess, output_size, mainNet, targetNet, batch_size)
...
while True:
    for _ in range(num_step):
        ...
        if np.random.rand() < e:
            ...
            actions = [agent.get_action([i])[0] for i in states]
