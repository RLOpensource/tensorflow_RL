...
lstm_shape = [512]
actor = CNNActorLSTM('actor', window_size, obs_stack, output_size, lstm_shape)
critic = CNNCriticLSTM('critic', window_size, obs_stack, lstm_shape)
agent = PPO(sess, output_size, None, None, actor, critic)
agent.epoch = 6
sess.run(tf.global_variables_initializer())
saver = tf.train.Saver()
saver.restore(sess, 'break_out_ppolstm/model')
...
for idx in range(num_worker):
    ...
    work = Environment(visualize if sample_idx == idx else visualize, idx, child_conn)
    work.start()
    works.append(work)
...
while True:
    ...
    saver.save(sess, 'break_out_ppolstm/model')